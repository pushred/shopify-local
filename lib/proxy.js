...
function cacheResponse (err, response, request, reply) {
  ...
  wreck.read(response, null, (err, body) => {
    ...
    // cache JSON as an object, hapi does not handle parsing in replies
    if (/json/.test(contentType) && typeof responseBody === 'string') responseBody = JSON.parse(responseBody);
